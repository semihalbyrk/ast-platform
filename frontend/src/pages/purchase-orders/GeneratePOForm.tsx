...
export default function GeneratePOForm() {
  ...
  return (
    <div className="max-w-lg">
      ...
      <h1 className="text-2xl font-bold text-grey-900">Generate Purchase Invoice</h1>
      <p className="text-sm text-grey-500 mt-1">Creates a Purchase Invoice from completed inbounds in the selected period.</p>
      ...
      <form onSubmit={handleSubmit} className="mt-6 space-y-4 bg-white rounded-lg border border-grey-200 shadow-sm p-6">
        ...
        <div><label className={labelCls}>Client (Opdrachtgever) <span className="text-red-500">*</span></label>
          <select required value={clientId} onChange={(e) => setClientId(e.target.value)} className={inputCls}>
            <option value="">Select client (opdrachtgever)...</option>{clients.map((c) => <option key={c.id} value={c.id}>{c.name}</option>)}</select></div>
        ...
        <div className="flex gap-3 pt-2">
          <button type="submit" disabled={submitting} className="h-9 px-4 bg-green-500 text-white rounded-md text-sm font-semibold hover:bg-green-700 disabled:opacity-50">{submitting ? 'Generating...' : 'Generate Purchase Invoice'}</button>
